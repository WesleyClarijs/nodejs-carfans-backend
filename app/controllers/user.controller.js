    ...
    await user
      ...
      .catch((err) => {
        res.status(500).send({
          message:
            err.message ||
            "Some error occured while adding the user to the database!",
        });
      });
  },
    ...
    await Users.find()
      ...
      .catch((err) => {
        res.status(500).send({
          message:
            err.message ||
            "Some error occured while retreiving all users from database!",
        });
      });
  },
    ...
    await Users.findById(id)
      ...
      .catch((err) => {
        res
          .status(500)
          .send({
            message: "Error retreiving User with id " + id + " from database!",
          });
      });
  },
    ...
    await Users.findByIdAndRemove(id)
      .then((data) => {
        if (!data) {
          res.status(404).send({
            message:
              "Can not delete User with id " + id + ". User was not found!",
          });
        } else {
          res.send({
